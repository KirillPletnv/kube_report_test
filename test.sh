...
#Create server name (default is "default_server")
...
#Create services file with errors (Error or CrashLoopBackOff)
grep -E 'Error|CrashLoopBackOff' "$ORIG_FILE" | \
awk '{print $1}' | \
sed -E 's/(-[^-]{6,10}-[^-]{5,6})$//' > "$FAILED_FILE"

#Create running services file (Running)
...
CURRENT_DATE=$(date +"%d/%m/%y")
{
    ...
    echo "Number of failed services: $FAILED_COUNT"
...
if tar -tzf "${ARCHIVE_DIR}/${ARCHIVE_FILE}" >/dev/null 2>&1; then
    echo "Archive created successfully: ${ARCHIVE_DIR}/${ARCHIVE_FILE}"
    exit 0
else
